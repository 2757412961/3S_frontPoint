...
<template>
    <el-form class="formclass" ref="form" :model="form">
        ...
        >
            ...
            >
        <span
                @click="showHidden($event)"
                :style="{position:'absolute',left:'40px',width:'200px',top:'10px',
                    'text-align':'left'}"
        >{{layerName}}</span>
            ...
            <polygonRender
                    v-if="(myMap.getLayer(layerName).type==='fill'||myMap.getLayer(layerName).type==='fill-extrusion')"
                    :layername="layerName"
                    :myMap="myMap"
                    @refreshStorage="refreshStorage"
            ></polygonRender>
            <pointRender
                    v-if="(myMap.getLayer(layerName).type==='circle'||myMap.getLayer(layerName).type==='symbol'||
        myMap.getLayer(layerName).type==='heatmap')"
            ...
            ></pointRender>
            <lineRenderer
                    v-if="(myMap.getLayer(layerName).type==='line')"
                    :layername="layerName"
                    :myMap="myMap"
                    @refreshStorage="refreshStorage"
            ></lineRenderer>
